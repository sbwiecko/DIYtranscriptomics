...
After read mapping with Kallisto, [TxImport](https://bioconductor.org/packages/release/bioc/html/tximport.html) was used to read kallisto outputs into the R environment.  Annotation data from Biomart was used to summarize data from transcript-level to gene-level. _Note that for the vignette the script step1.R isn't necessary as the target sample list and DGEList have been already included in the data set of the package + there is no kallisto output available._
...
#myDGEList <- DGEList(Txi_gene$counts) no count available
